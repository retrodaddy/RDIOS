...
| People | Prototype | Institutions | Live, mock-backed — Position/Affiliation/Capability, roster + profile screens, multi-parent since M4 | Authority & Permissions | M5 | Membership-to-multiple-Positions question named open in the People Domain Review — still open |
| Organization (Builder) | Prototype | People | Live, mock-backed — canvas at `/people/organization`, drag-to-move, drag-to-connect, multi-parent | Cycle detection, node deletion | M5 | No cycle guard yet — a Position could in principle be connected into a cycle through several drags; named honestly in the Technical Debt Register, not yet a real-world problem at current institution sizes |
...
| No cycle detection in the Position reporting graph | `applications/people/mock-provider.ts` (`updatePositionParents`) | New from M4. Only a direct self-loop is blocked (`id !== positionId`); nothing stops A→B→A being built one drag at a time. Not yet a real-world problem — the canvas UI makes a multi-step cycle awkward to build by accident — but a real gap the multi-parent graph introduced that single-parent trees structurally couldn't have. |
...
| Role-based navigation is not implemented — every member sees the identical sidebar regardless of what they hold | `components/os/Shell.tsx`, `os/navigation/index.ts` | Deliberately deferred. The one honest "role" signal that exists without inventing new architecture — the signed-in person's current Position — is now surfaced in the sidebar footer as a real personalization, but the nav *destinations themselves* don't change per person; that needs Authority & Permissions (M5) to mean anything real, not a throwaway placeholder gate built ahead of schedule. |
...
**High**
- Extract shared UI primitives — the single most overdue item in the register; two milestones running have repeated the same hand-built pattern.
...
- Cycle detection for the Position reporting graph.
- Replace People's single-select Position picker with a searchable one before institution sizes make it unusable.
- Real role-based navigation, once Authority & Permissions (M5) gives "role" an actual meaning to gate on.
